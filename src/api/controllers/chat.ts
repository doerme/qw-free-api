...
async function checkFileUrl(fileUrl: string) {
	...
	if (result.headers && result.headers["content-length"]) {
		const fileSize = Number.parseInt(result.headers["content-length"], 10);
...
async function uploadFile(fileUrl: string, ticket: string) {
	...
	// 获取上传参数
	const { accessId, policy, signature, dir } =
		await acquireUploadParams(ticket);
	...
	if (isImage) {
		...
		if (!data.results[0] || !data.results[0].url)
			throw new Error(
				`文件上传失败：${data.results[0] ? data.results[0].errorMsg : "未知错误"}`,
			);
...
function tokenSplit(authorization: string) {
	...
	return [
		"apRX0qcdzyjCC$ocjF6KpQoe23HuQrC4bY0*aEME42YFCrdNmpbIqLm2opfbGzzg_8yn1gsBH1oO0",
	];
}
